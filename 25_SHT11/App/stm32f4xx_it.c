...
void EXTI9_5_IRQHandler(void)
{
	...
	if(GPIO_ReadInputDataBit(GPIO_PORT_ExtInt4,GPIO_PIN_ExtInt4))
	{
	}		 
	EXTI_ClearITPendingBit(EXTI_Line9);//只允许一个 Port 的 9引脚中断！  
}
...
{
	if(GPIO_ReadInputDataBit(GPIO_PORT_ExtInt1,GPIO_PIN_ExtInt1))
	{				 
		BSP_Delay_ms(10);
		if(GPIO_ReadInputDataBit(GPIO_PORT_ExtInt1,GPIO_PIN_ExtInt1))
		{
	...
	{		
		...
		{
			printf("烟雾报警！！！\r\n");
			BSP_LEDBuzzer_On(Buzzer);
		}
		
	}
	EXTI_ClearITPendingBit(EXTI_Line13);//只允许一个 Port 的 13引脚中断！
}
/**
  * @}
  */ 
